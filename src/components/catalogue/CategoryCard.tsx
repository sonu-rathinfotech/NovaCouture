...
export function CategoryCard({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-gradient-to-t from-noir/85 via-noir/35 to-transparent"
      />

      <div className="absolute inset-x-0 bottom-0 p-8 text-left lg:p-10">
        {sub && (
          <p className="mb-2 text-[0.65rem] tracking-[0.25em] text-champagne-300/90 uppercase opacity-0 transition-opacity duration-500 group-hover:opacity-100">
            {sub}
          </p>
        )}
        <h3
          className={`font-serif text-charcoal-900 transition-colors duration-500 group-hover:text-champagne-400 ${
            large ? 'text-4xl lg:text-5xl' : 'text-3xl'
          }`}
        >
          {category.name}
        </h3>
        <span className="mt-4 inline-flex items-center gap-2 text-[0.7rem] tracking-[0.2em] text-champagne-300/0 uppercase transition-all duration-500 group-hover:gap-3 group-hover:text-champagne-300/90">
          Explore
          <span
            aria-hidden="true"
            className="block h-px w-0 bg-champagne-300/80 transition-all duration-500 group-hover:w-8"
          />
        </span>
      </div>
    </Link>
